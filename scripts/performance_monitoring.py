    ...
    "dashboard": {
        ...
        "panels": [
            ...
            },
            {
                ...
                "type": "stat",
                "targets": [
                    {"expr": "rate(perf_test_errors_total[5m])", "legendFormat": "{{error_type}}"}
                ],
                "fieldConfig": {
                    "defaults": {
                        "unit": "percent",
            ...
            "rules": [
                {
                    ...
                    "for": "2m",
                    "labels": {"severity": "warning", "service": "{{ $labels.endpoint }}"},
                    "annotations": {
                ...
                },
                {
                    ...
                    "for": "5m",
                    "labels": {"severity": "critical", "service": "{{ $labels.endpoint }}"},
                    "annotations": {
...
def generate_performance_summary(results_dir: Path) -> dict:
            ...
            summary["test_runs"].append(
                {
                    ...
                    "service": (
                        metrics.get("test_name", "").split("_")[-1]
                        if "_" in metrics.get("test_name", "")
                        else "unknown"
                    ),
                    "total_requests": metrics.get("total_requests", 0),
                    "success_rate": (
                        metrics.get("successful_requests", 0) / metrics.get("total_requests", 1)
                    )
                    * 100,
    ...
    if summary["test_runs"]:
        total_successful = sum(
            run["total_requests"] * run["success_rate"] / 100 for run in summary["test_runs"]
        )
        summary["overall_success_rate"] = (
            (total_successful / summary["total_requests"]) * 100
            if summary["total_requests"] > 0
            else 0
        )
        summary["average_response_time"] = sum(
            run["average_response_time"] for run in summary["test_runs"]
        ) / len(summary["test_runs"])
