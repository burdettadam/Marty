...
class K8sValidationTest:
    ...
    def test_cluster_connectivity(self) -> bool:
        ...
        if success:
            ...
            self.results.append(("Cluster Connectivity", True, "Successfully connected to Kind cluster"))
            return True
        else:
    ...
    def test_namespace_operations(self) -> bool:
        ...
        # Create namespace
        success, stdout, stderr = self.run_command([
            "kubectl", "create", "namespace", self.namespace, "--context", f"kind-{self.cluster_name}"
        ])
        
        if not success and "already exists" not in stderr:
            logger.error(f"❌ Namespace creation: FAIL - {stderr}")
            self.results.append(("Namespace Operations", False, f"Failed to create namespace: {stderr}"))
            return False
        
        # List namespace to verify it exists
        success, stdout, stderr = self.run_command([
            "kubectl", "get", "namespace", self.namespace, "--context", f"kind-{self.cluster_name}"
        ])
        
        if not success:
            logger.error(f"❌ Namespace listing: FAIL - {stderr}")
            self.results.append(("Namespace Operations", False, f"Failed to list namespace: {stderr}"))
            return False
        
        logger.info("✅ Namespace operations: PASS")
        self.results.append(("Namespace Operations", True, "Successfully created and listed namespace"))
        return True
    ...
    def test_protobuf_compilation(self) -> bool:
        ...
        if success and ("Successfully compiled proto files" in stdout or "Fixed imports" in stdout):
            ...
            self.results.append(("Protobuf Compilation", True, "All proto files compiled successfully"))
            return True
    ...
    def test_simple_pod_deployment(self) -> bool:
        ...
        # Deploy nginx pod
        success, stdout, stderr = self.run_command([
            "kubectl", "run", "validation-test-nginx",
            "--image=nginx:alpine",
            "--restart=Never",
        ...
        for attempt in range(30):  # 30 seconds timeout
            success, stdout, stderr = self.run_command([
                "kubectl", "get", "pod", "validation-test-nginx",
                f"--namespace={self.namespace}",
                "--context", f"kind-{self.cluster_name}",
            ...
            if success and "Running" in stdout:
                ...
                self.results.append(("Pod Deployment", True, "Successfully deployed and started nginx pod"))
                return True
    ...
    def test_service_creation(self) -> bool:
        ...
        success, stdout, stderr = self.run_command([
            "kubectl", "expose", "pod", "validation-test-nginx",
            "--port=80",
            "--target-port=80",
            f"--namespace={self.namespace}",
            "--context", f"kind-{self.cluster_name}"
        ...
        success, stdout, stderr = self.run_command([
            "kubectl", "get", "service", "validation-test-nginx",
            f"--namespace={self.namespace}",
            "--context", f"kind-{self.cluster_name}"
        ])
        ...
        if success:
            ...
            self.results.append(("Service Creation", True, "Successfully created Kubernetes service"))
            return True
        else:
            logger.error(f"❌ Service creation: FAIL - {stderr}")
            self.results.append(("Service Creation", False, f"Service verification failed: {stderr}"))
            return False
    ...
    def test_helm_chart_validation(self) -> bool:
        ...
        for chart_path in chart_paths:
            ...
            if not success:
                ...
                self.results.append(("Helm Chart Validation", False, f"Chart {chart_path} failed validation: {stderr}"))
                return False
    ...
    def test_k8s_orchestrator_import(self) -> bool:
        ...
        if success and "Import successful" in stdout:
            ...
            self.results.append(("K8s Orchestrator Import", True, "Test orchestrator imports correctly"))
            return True
        else:
