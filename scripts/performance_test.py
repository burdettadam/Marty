...
REQUEST_COUNT = Counter(
    "perf_test_requests_total", "Total requests made", ["endpoint", "method", "status"]
)
REQUEST_DURATION = Histogram(
    "perf_test_request_duration_seconds", "Request duration", ["endpoint", "method"]
)
...
class PerformanceTestFramework:
    ...
    async def load_test(
        ...
        """
        logger.info(
            f"Starting load test for {service_name}: {concurrent_users} users, {duration_seconds}s"
        )
    ...
    async def stress_test(
        self, service_name: str, max_users: int = 100, ramp_up_time: int = 300
    ) -> list[TestResult]:
        """
        ...
        """
        logger.info(
            f"Starting stress test for {service_name}: ramping up to {max_users} users over {ramp_up_time}s"
        )
    ...
    def save_results(
        self, results: list[TestResult], test_name: str, metrics: PerformanceMetrics
    ) -> Path:
...
async def run_load_test(
    service_name: str, concurrent_users: int = 10, duration: int = 60, rps: int | None = None
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "service", choices=["pkd_service", "document_processing", "ui_app"], help="Service to test"
    )
