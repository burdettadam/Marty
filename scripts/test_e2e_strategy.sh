...
# This script provides a comprehensive testing strategy that ensures proper service setup
...
start_services() {
    ...
    timeout=30
    while ! docker-compose exec -T postgres pg_isready -U martyuser -d martydb >/dev/null 2>&1; do
        sleep 1
        timeout=$((timeout - 1))
    ...
    docker-compose up -d trust-anchor
    
    echo "🔧 Starting CSCA service..."  
    docker-compose up -d csca-service
    
    echo "🔧 Starting document signer..."
    docker-compose up -d document-signer
    
    echo "🔧 Starting inspection system..."
    docker-compose up -d inspection-system
    
    echo "🔧 Starting passport engine..."
    docker-compose up -d passport-engine
    
    echo "🔧 Starting MDL engine..."
    docker-compose up -d mdl-engine
    
    echo "🔧 Starting mDoc engine..."
    docker-compose up -d mdoc-engine
    
    echo "🔧 Starting DTC engine..."
    docker-compose up -d dtc-engine
