...
class CertificateInfo:
    ...
    def __init__(self):
        ...
        self.public_key_algorithm: Optional[str] = None
        self.public_key_size: Optional[int] = None
        self.fingerprint_sha1: Optional[str] = None
        self.fingerprint_sha256: Optional[str] = None
        self.fingerprint_md5: Optional[str] = None
        self.key_usage: List[str] = []
        self.extended_key_usage: List[str] = []
        self.basic_constraints: Optional[Dict[str, Any]] = None
        self.subject_alternative_names: List[str] = []
        self.issuer_alternative_names: List[str] = []
        self.authority_key_identifier: Optional[str] = None
        self.subject_key_identifier: Optional[str] = None
        ...
        self.path_length_constraint: Optional[int] = None
        self.raw_der: Optional[bytes] = None
...
class ASN1Parser:
    ...
    @staticmethod
    def parse_master_list_identifier(extension_value: bytes) -> Optional[str]:
    ...
    @staticmethod
    def parse_country_identifier(extension_value: bytes) -> Optional[str]:
...
class X509CertificateParser:
    ...
    def parse_certificate(self, cert_data: Union[bytes, str]) -> CertificateInfo:
    ...
    def _normalize_certificate_data(self, cert_data: Union[bytes, str]) -> bytes:
    ...
    def _process_extension(self, extension: x509.Extension, cert_info: CertificateInfo):
        ...
        # ICAO Extensions
        elif oid in [ICAOExtensions.DOCUMENT_TYPE_LIST, 
...
class CertificateValidator:
    ...
    def add_trusted_csca(self, csca_cert: Union[bytes, str], identifier: str = None):
    ...
    def validate_certificate(self, cert_data: Union[bytes, str], 
                           issuer_cert: Optional[Union[bytes, str]] = None) -> Tuple[ValidationResult, Dict[str, Any]]:
    ...
    def _check_time_validity(self, cert_info: CertificateInfo, 
    ...
    def _verify_signature(self, cert_data: bytes, issuer_cert_data: Union[bytes, str],
                         validation_details: Dict[str, Any]) -> ValidationResult:
    ...
    def _validate_chain(self, cert_info: CertificateInfo, 
                       validation_details: Dict[str, Any]) -> ValidationResult:
        ...
        if cert_info.certificate_type == CertificateType.DSC:
            ...
            if cert_info.authority_key_identifier:
                for csca in self.trusted_cscas.values():
                    if (csca.subject_key_identifier == cert_info.authority_key_identifier or
                        csca.subject == cert_info.issuer):
                        return ValidationResult.VALID
    ...
    def _validate_icao_compliance(self, cert_info: CertificateInfo, 
...
class CertificateChainBuilder:
    ...
    def add_certificate(self, cert_data: Union[bytes, str], identifier: str = None) -> str:
    ...
    def _find_issuer(self, cert: CertificateInfo) -> Optional[CertificateInfo]:
        ...
        for candidate in self.certificates.values():
            ...
            if candidate.subject == cert.issuer:
                ...
                if (cert.authority_key_identifier and 
                    candidate.subject_key_identifier == cert.authority_key_identifier):
                    return candidate
