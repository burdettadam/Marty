...
from vault_client import TrustServiceVaultClient, VaultSecretError, get_vault_client
...
class RotatingCredentialManager:
    ...
    def __init__(self, config: SecureDatabaseConfig):
        self.config = config
        self.current_credentials: dict[str, Any] | None = None
        self.next_credentials: dict[str, Any] | None = None
...
class SecureDatabaseManager:
    ...
    def __init__(self):
        ...
        self.credential_manager: RotatingCredentialManager | None = None
        self.engine: AsyncEngine | None = None
        self.session_factory: async_sessionmaker[AsyncSession] | None = None
        self.connection_monitor: asyncio.Task | None = None

        # Security monitoring
        self.connection_attempts: int = 0
        self.failed_connections: int = 0
        self.last_security_check: datetime | None = None
        ...
        while True:
            try:
                ...
                if self.engine:
                    ...
                    # Log pool statistics
                    logger.debug(
                        f"Connection pool stats - Size: {pool.size()}, Checked out: {pool.checkedout()}"
                    )

                    # Check for suspicious activity
                    if self.failed_connections > 10:  # Threshold
                        logger.warning(
                            f"High number of failed connections: {self.failed_connections}"
                        )
        ...
        try:
            async with self.session_factory() as session:
                ...
                await session.execute(
                    text("SET SESSION application_name = 'trust-service-session'")
                )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        health_info = {
            ...
            "last_check": (
                self.last_security_check.isoformat() if self.last_security_check else None
            ),
            "credentials_rotated": (
                self.credential_manager.last_rotation.isoformat()
                if self.credential_manager.last_rotation
                else None
            ),
        }
...
_secure_db_manager: SecureDatabaseManager | None = None
