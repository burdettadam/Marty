...
from .models import CRL, CSCA, DSC, MasterList, Provenance, Source
from .schemas import (
    ApiResponse,
    TrustAnchor,
...
async def get_trust_status(
    ...
    try:
        ...
        csca_query = select(
            CSCA.country_code, CSCA.status, func.count(CSCA.id).label("count")
        ...
        dsc_query = select(
            DSC.country_code, DSC.status, func.count(DSC.id).label("count")
...
async def get_trust_snapshot(
    ...
    try:
        ...
        for csca in cscas:
            ...
            dsc_query = select(DSC).where(and_(*dsc_conditions)).order_by(DSC.created_at)
            dsc_result = await session.execute(dsc_query)
...
async def get_trust_anchors(
    ...
    status_filter: str | None = Query("active", description="Filter by status"),
    ...
    try:
        ...
        # Get total count
        count_query = select(func.count(CSCA.id)).where(and_(*conditions) if conditions else True)
