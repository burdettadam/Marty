...
from .metrics import (
    record_pkd_sync,
    update_master_list_age,
    update_trusted_csca_count,
    update_trusted_dsc_count,
)
from .models import CRL, CSCA, DSC, MasterList, Provenance, Source, SourceType
...
class PKDIngestionService:
    ...
    async def sync_all_sources(self) -> dict[str, list[str]]:
        ...
        for source in sources:
            try:
                ...
                record_pkd_sync(
                    source.source_type, source.country_code or "global", "success", duration
                )
    ...
    async def _process_master_list_data(self, source: Source, data: bytes) -> None:
        ...
        existing_query = select(MasterList).where(
            MasterList.content_hash == content_hash, MasterList.country_code == source.country_code
        )
            ...
            for cert_block in cert_blocks:
                try:
                    ...
                    if validation_result["certificate_info"]:
                        ...
                        cert_type = (
                            CertificateType.CSCA if cert_info["is_ca"] else CertificateType.DSC
                        )
    ...
    async def _process_certificates(
        self, master_list: MasterList, certificates: list[dict]
    ) -> None:
        ...
        for cert_entry in certificates:
            try:
                ...
                if cert_id:
                    ...
                    logger.info(
                        f"Stored {cert_type.value} certificate: {cert_info.get('subject', 'Unknown')}"
                    )
                else:
                    logger.warning("Failed to store certificate or already exists")
    ...
    try:
        async for session in get_async_session():
            async with PKDIngestionService(session) as pkd_service:
                ...
                logger.info(
                    f"Sync completed - Success: {len(results['success'])}, Failed: {len(results['failed'])}"
                )
