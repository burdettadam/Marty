...
class JWTAuthenticator:
    ...
    def generate_token(
        ...
        token = jwt.encode(payload, self._private_key, algorithm=self.config.jwt_algorithm)
        ...
        def wrapper(request, context: _Context):
            try:
                ...
                if client_cn not in self.mtls_auth._allowed_clients:
                    context.abort(
                        StatusCode.PERMISSION_DENIED, f"Client '{client_cn}' not authorized"
                    )

                # Add client info to context
                context.set_trailing_metadata([("authenticated-client", client_cn)])
...
def get_client_certificate_info(context: _Context) -> dict[str, Any] | None:
    ...
    try:
        ...
        if "x509_subject_alternative_name" in auth_context:
            cert_info["san"] = [
                san.decode("utf-8") for san in auth_context["x509_subject_alternative_name"]
            ]
