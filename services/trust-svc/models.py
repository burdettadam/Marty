...
from sqlalchemy import (
    Boolean, DateTime, Integer, String, Text, LargeBinary, 
...
class MasterList(Base):
    ...
    signature_data: Mapped[Optional[bytes]] = mapped_column(LargeBinary)
...
class CSCA(Base):
    ...
    valid_from: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    valid_to: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    key_usage: Mapped[Optional[list[str]]] = mapped_column(ARRAY(String))
    signature_algorithm: Mapped[Optional[str]] = mapped_column(String(50))
    public_key_algorithm: Mapped[Optional[str]] = mapped_column(String(50))
    ...
    master_list_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey('trust_svc.master_lists.id'))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class DSC(Base):
    ...
    certificate_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    certificate_data: Mapped[bytes] = mapped_column(LargeBinary, nullable=False)
    issuer_csca_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey('trust_svc.cscas.id'))
    subject_dn: Mapped[str] = mapped_column(Text, nullable=False)
    issuer_dn: Mapped[str] = mapped_column(Text, nullable=False)
    serial_number: Mapped[str] = mapped_column(Text, nullable=False)
    valid_from: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    ...
    revocation_date: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    master_list_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey('trust_svc.master_lists.id'))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class CRL(Base):
    ...
    revoked_certificates: Mapped[Optional[dict]] = mapped_column(JSONB)  # Serialized revocation entries
    source_url: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class Source(Base):
    ...
    credentials: Mapped[Optional[dict]] = mapped_column(JSONB)  # Encrypted credentials
...
class Provenance(Base):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    entity_type: Mapped[str] = mapped_column(String(50), nullable=False)  # 'master_list', 'csca', 'dsc', 'crl'
    entity_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), nullable=False)
    source_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey('trust_svc.sources.id'), nullable=False)
    operation: Mapped[str] = mapped_column(String(20), nullable=False)  # 'create', 'update', 'delete'
    changes: Mapped[Optional[dict]] = mapped_column(JSONB)  # What changed
    checksum: Mapped[str] = mapped_column(String(64), nullable=False)
    signature: Mapped[Optional[str]] = mapped_column(Text)  # Digital signature of the change
    metadata: Mapped[Optional[dict]] = mapped_column(JSONB)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
