...
class TrustServiceVaultClient:
    ...
    async def get_secret(self, path: str, key: Optional[str] = None) -> Union[Dict[str, Any], Any]:
        """
        ...
        try:
            response = self.client.secrets.kv.v2.read_secret_version(
                path=path,
                mount_point=self.mount_point_kv
            )
        ...
        try:
            ...
            try:
                response = self.client.secrets.pki.read_certificate(
                    ...
                    mount_point=self.mount_point_pki
                )
...
_vault_client: Optional[TrustServiceVaultClient] = None
