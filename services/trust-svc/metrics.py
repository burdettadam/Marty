...
TRUSTED_DSC_TOTAL = Gauge(
    "trusted_dsc_total", "Total number of trusted DSC certificates", ["country_code", "status"]
)
...
PKD_SYNC_DURATION = Histogram(
    "pkd_sync_duration_seconds", "Duration of PKD sync operations", ["source_type", "country_code"]
)
...
CRL_REFRESH_DURATION = Histogram(
    "crl_refresh_duration_seconds", "Duration of CRL refresh operations", ["country_code"]
)
...
DATABASE_CONNECTION_POOL = Gauge(
    "database_connection_pool_size", "Current database connection pool size", ["pool_type"]
)
...
API_REQUESTS_TOTAL = Counter(
    "api_requests_total", "Total number of API requests", ["method", "endpoint", "status_code"]
)

GRPC_REQUEST_DURATION = Histogram(
    "grpc_request_duration_seconds", "Duration of gRPC requests", ["method", "status"]
)

GRPC_REQUESTS_TOTAL = Counter(
    "grpc_requests_total", "Total number of gRPC requests", ["method", "status"]
)
...
# Error metrics
ERROR_TOTAL = Counter("errors_total", "Total number of errors", ["error_type", "component"])

# Health metrics
SERVICE_UPTIME = Gauge("service_uptime_seconds", "Service uptime in seconds")

SERVICE_HEALTH = Gauge(
    "service_health_status", "Service health status (1=healthy, 0=unhealthy)", ["component"]
)
...
def update_master_list_age(country_code: str, source_type: str, age_seconds: float) -> None:
    ...
    MASTER_LIST_AGE_SECONDS.labels(country_code=country_code, source_type=source_type).set(
        age_seconds
    )
...
def update_trusted_csca_count(country_code: str, trust_level: str, status: str, count: int) -> None:
    ...
    TRUSTED_CSCA_TOTAL.labels(
        country_code=country_code, trust_level=trust_level, status=status
    ).set(count)


def update_trusted_dsc_count(country_code: str, status: str, count: int) -> None:
    """Update trusted DSC count metric."""
    TRUSTED_DSC_TOTAL.labels(country_code=country_code, status=status).set(count)


def record_pkd_sync(
    source_type: str, country_code: str, status: str, duration: float | None = None
) -> None:
    """Record PKD sync operation."""
    PKD_SYNC_OPERATIONS.labels(
        source_type=source_type, country_code=country_code, status=status
    ).inc()

    if duration is not None:
        PKD_SYNC_DURATION.labels(source_type=source_type, country_code=country_code).observe(
            duration
        )


def record_crl_refresh(country_code: str, status: str, duration: float | None = None) -> None:
    """Record CRL refresh operation."""
    CRL_REFRESH_OPERATIONS.labels(country_code=country_code, status=status).inc()

    if duration is not None:
        CRL_REFRESH_DURATION.labels(country_code=country_code).observe(duration)
...
def record_api_request(method: str, endpoint: str, status_code: int, duration: float) -> None:
    ...
    API_REQUEST_DURATION.labels(method=method, endpoint=endpoint, status_code=status_code).observe(
        duration
    )
...
def record_certificate_validation(
    cert_type: str, country_code: str, validation_result: str, duration: float
) -> None:
    """Record certificate validation metrics."""
    CERTIFICATE_VALIDATIONS.labels(
        cert_type=cert_type, country_code=country_code, validation_result=validation_result
    ).inc()

    CERTIFICATE_VALIDATION_DURATION.labels(cert_type=cert_type, country_code=country_code).observe(
        duration
    )
