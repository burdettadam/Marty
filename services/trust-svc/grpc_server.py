...
from .models import CRL, CSCA, DSC, CertificateStatus, MasterList, Source, SourceType, TrustLevel
...
class TrustServiceGRPC(
    trust_service_pb2_grpc.TrustServiceServicer if trust_service_pb2_grpc else object
):
        ...
        try:
            async for session in get_async_session():
                ...
                if request.country_code:
                    ml_query = ml_query.where(
                        MasterList.country_code == request.country_code.upper()
                    )
                ...
                # Get CSCA counts
                csca_query = select(func.count(CSCA.id)).where(
                    CSCA.status == CertificateStatus.ACTIVE
                )
                if request.country_code:
    ...
    async def GetTrustAnchors(self, request, context):
        ...
        try:
            async for session in get_async_session():
                ...
                if (
                    request.trust_level
                    and request.trust_level != trust_service_pb2.TRUST_LEVEL_UNSPECIFIED
                ):
                    trust_level_map = {
                ...
                if (
                    request.status
                    and request.status != trust_service_pb2.CERTIFICATE_STATUS_UNSPECIFIED
                ):
                    status_map = {
                ...
                csca_query = (
                    csca_query.order_by(CSCA.country_code, CSCA.created_at)
                    .limit(page_size)
                    .offset(offset)
                )
                ...
                for csca in cscas:
                    ...
                    anchor = trust_service_pb2.TrustAnchor(
                        ...
                        trust_level=trust_level_map.get(
                            csca.trust_level, trust_service_pb2.TRUST_LEVEL_STANDARD
                        ),
                        status=status_map.get(
                            csca.status, trust_service_pb2.CERTIFICATE_STATUS_ACTIVE
                        ),
                    ...
                    if (
                        hasattr(request, "include_certificate_data")
                        and request.include_certificate_data
                    ):
        ...
        try:
            ...
            async for session in get_async_session():
                ...
                response = trust_service_pb2.ValidateCertificateResponse(
                    is_valid=is_valid,
                    revocation_status=revocation_status_map.get(
                        revocation_status, trust_service_pb2.REVOCATION_STATUS_UNKNOWN
                    ),
    ...
    async def GetDataSources(self, request, context):
        ...
        try:
            async for session in get_async_session():
                ...
                if (
                    request.source_type
                    and request.source_type != trust_service_pb2.SOURCE_TYPE_UNSPECIFIED
                ):
                    source_type_map = {
                ...
                for source in sources:
                    ...
                    pb_source = trust_service_pb2.DataSource(
                        ...
                        source_type=source_type_map.get(
                            source.source_type, trust_service_pb2.SOURCE_TYPE_UNSPECIFIED
                        ),
