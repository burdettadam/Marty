...
def upgrade() -> None:
    ...
    op.create_index('idx_master_lists_country_version', 'master_lists', ['country_code', 'version'], schema='trust_svc')
    op.create_index('idx_master_lists_source_updated', 'master_lists', ['source_type', 'updated_at'], schema='trust_svc')
    ...
    op.create_unique_constraint('uq_cscas_certificate_hash', 'cscas', ['certificate_hash'], schema='trust_svc')
    op.create_index('idx_cscas_country_status', 'cscas', ['country_code', 'status'], schema='trust_svc')
    op.create_index('idx_cscas_validity', 'cscas', ['valid_from', 'valid_to'], schema='trust_svc')
    op.create_foreign_key('fk_cscas_master_list', 'cscas', 'master_lists', ['master_list_id'], ['id'], source_schema='trust_svc', referent_schema='trust_svc')
    ...
    op.create_unique_constraint('uq_dscs_certificate_hash', 'dscs', ['certificate_hash'], schema='trust_svc')
    op.create_index('idx_dscs_country_status', 'dscs', ['country_code', 'status'], schema='trust_svc')
    ...
    op.create_foreign_key('fk_dscs_issuer_csca', 'dscs', 'cscas', ['issuer_csca_id'], ['id'], source_schema='trust_svc', referent_schema='trust_svc')
    op.create_foreign_key('fk_dscs_master_list', 'dscs', 'master_lists', ['master_list_id'], ['id'], source_schema='trust_svc', referent_schema='trust_svc')
    ...
    op.create_index('idx_crls_issuer_country', 'crls', ['issuer_dn', 'country_code'], schema='trust_svc')
    ...
    op.create_unique_constraint('uq_sources_type_country_url', 'sources', ['source_type', 'country_code', 'url'], schema='trust_svc')
    op.create_index('idx_sources_type_status', 'sources', ['source_type', 'is_active'], schema='trust_svc')
    ...
    op.create_index('idx_provenance_entity', 'provenance', ['entity_type', 'entity_id'], schema='trust_svc')
    op.create_index('idx_provenance_source', 'provenance', ['source_id', 'created_at'], schema='trust_svc')
    op.create_foreign_key('fk_provenance_source', 'provenance', 'sources', ['source_id'], ['id'], source_schema='trust_svc', referent_schema='trust_svc')
    
    # Create constraints for enum-like fields
    op.execute("ALTER TABLE trust_svc.cscas ADD CONSTRAINT valid_trust_level CHECK (trust_level IN ('standard', 'high', 'emergency'))")
    op.execute("ALTER TABLE trust_svc.cscas ADD CONSTRAINT valid_status CHECK (status IN ('active', 'inactive', 'revoked'))")
    op.execute("ALTER TABLE trust_svc.dscs ADD CONSTRAINT valid_dsc_status CHECK (status IN ('active', 'inactive', 'revoked'))")
