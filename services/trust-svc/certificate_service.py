...
from .certificate_parser import (
    CertificateInfo, CertificateType, CertificateValidator, 
    ValidationResult, X509CertificateParser, CertificateChainBuilder
)
from .database import get_async_session
from .models import CSCA, DSC, CRL, CertificateStatus, TrustLevel
...
class TrustServiceCertificateValidator:
                ...
                if csca.certificate_data:
                    try:
                        self.validator.add_trusted_csca(
                            csca.certificate_data, 
                            identifier=str(csca.id)
                        )
    ...
    async def validate_certificate_data(self, cert_data: Union[bytes, str], 
    ...
    async def validate_certificate_chain(self, cert_chain: List[Union[bytes, str]],
                                        session: Optional[AsyncSession] = None) -> Dict:
        ...
        try:
            ...
            for i in range(len(cert_chain)):
                if i < len(cert_chain) - 1:
                    ...
                    result = await self.validate_certificate_data(
                        cert_chain[i], session=session
                    )
                    # Add chain-specific validation
                    issuer_validation = self._validate_issuer_relationship(
                        chain_info[i]["certificate_info"],
                        chain_info[i + 1]["certificate_info"]
                    )
    ...
    async def parse_and_store_certificate(self, cert_data: Union[bytes, str],
    ...
    async def _store_certificate_in_db(self, cert_info: CertificateInfo, 
                                     cert_data: Union[bytes, str],
                                     certificate_type: CertificateType,
        ...
        try:
            if certificate_type == CertificateType.CSCA:
                ...
                csca = CSCA(
                    ...
                    certificate_data=cert_data if isinstance(cert_data, bytes) else cert_data.encode(),
    ...
    async def _check_revocation_status(self, cert_info: CertificateInfo, 
                                     session: AsyncSession) -> str:
        """Check certificate revocation status against CRLs."""
        try:
            # Query relevant CRLs
            crl_query = select(CRL).where(
                CRL.country_code == cert_info.country_code,
                CRL.is_active == True
            )
    ...
    def _validate_issuer_relationship(self, cert_info: Dict, issuer_info: Dict) -> Dict:
        ...
        validation["subject_issuer_match"] = (
            cert_info.get("issuer") == issuer_info.get("subject")
        )
        
        # Check key identifiers
        if (cert_info.get("authority_key_identifier") and 
            issuer_info.get("subject_key_identifier")):
            validation["key_identifier_match"] = (
                cert_info["authority_key_identifier"] == 
                issuer_info["subject_key_identifier"]
            )
