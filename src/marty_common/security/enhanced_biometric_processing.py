    ...
    def get_image(self) -> Image.Image:
        ...
        try:
            ...
            if image.size != (self.width, self.height):
                logger.warning(f"Image dimensions mismatch: expected {self.width}x{self.height}, got {image.size}")
...
class FingerprintTemplate:
    ...
    def _extract_iso_minutiae(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            minutiae_count = min(struct.unpack('>H', self.template_data[offset:offset+2])[0], 100)
            offset += 2
            ...
            for i in range(minutiae_count):
                ...
                minutia = {
                    ...
                    'type': 'ridge_ending' if (angle_type & 0x00C0) == 0x0040 else 'ridge_bifurcation',
    ...
    def extract_iris_features(self) -> dict[str, Any]:
        ...
        try:
            features = {
                ...
                'pupil_iris_ratio': self.pupil_radius / self.iris_radius if self.iris_radius > 0 else 0,
                'template_size': len(self.template_data),
...
class PassportBiometricProcessor:
    ...
    def __init__(self, enable_quality_assessment: bool = True,
    ...
    def process_dg2_face(self, dg2_data: bytes) -> FaceTemplate:
        ...
        try:
            ...
            self._log_processing("DG2", f"Face template processed successfully: {width}x{height} {image_format.value}")
    ...
    def process_dg3_fingerprint(self, dg3_data: bytes) -> list[FingerprintTemplate]:
        ...
        try:
            ...
            while offset < len(dg3_data) - 20:
                # Parse CBEFF header
                header = self._parse_cbeff_header(dg3_data[offset:offset+20], BiometricType.FINGER)
                offset += 20
                ...
                width, height, resolution = self._extract_fingerprint_metadata(template_data, fp_format)
    ...
    def process_dg4_iris(self, dg4_data: bytes) -> list[IrisTemplate]:
        ...
        try:
            ...
            while offset < len(dg4_data) - 20:
                # Parse CBEFF header
                header = self._parse_cbeff_header(dg4_data[offset:offset+20], BiometricType.IRIS)
                offset += 20
    ...
    def process_dg5_portrait(self, dg5_data: bytes) -> FaceTemplate:
        ...
        try:
            ...
            self._log_processing("DG5", f"Portrait processed: {width}x{height} {image_format.value}")
    ...
    def _extract_fingerprint_metadata(self, template_data: bytes, fmt: FingerprintFormat) -> tuple[int, int, int]:
    ...
    def _extract_iris_metadata(self, template_data: bytes) -> tuple[int, str, float]:
        ...
        try:
            if len(template_data) >= 16:
                ...
                diameter = struct.unpack('>H', template_data[8:10])[0] if len(template_data) >= 10 else 200
                rotation = struct.unpack('>f', template_data[12:16])[0] if len(template_data) >= 16 else 0.0
    ...
    def get_processing_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_processing": self.processing_log[-1]["timestamp"] if self.processing_log else None
        }
...
class MockBiometricData:
    ...
    def create_mock_dg2_data() -> bytes:
        ...
        header = struct.pack('>HHHHHHHHHH', 0x001B, 0x0201, 0x0002, 0x0000, 
    ...
    def create_mock_dg3_data() -> bytes:
        ...
        header = struct.pack('>HHHHHHHHHH', 0x001B, 0x0101, 0x0008, 0x0001,
                           0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000)
        
        template_length = struct.pack('>I', 100)
        
        # Simplified ISO fingerprint template
        iso_template = (b'FMR\x00\x20\x20\x00\x00\x01\x00\x01\x00\x00\x01\x00'
...
if __name__ == "__main__":
    ...
    try:
        ...
        if face_template.quality_score:
            print(f"  Quality: {face_template.quality_score.quality_level.value} ({face_template.quality_score.overall_score}/100)")
            print(f"  Issues: {', '.join(face_template.quality_score.quality_issues) if face_template.quality_score.quality_issues else 'None'}")
        ...
        for i, fp_template in enumerate(fp_templates):
            ...
            if fp_template.quality_score:
                print(f"    Quality: {fp_template.quality_score.quality_level.value} ({fp_template.quality_score.overall_score}/100)")
