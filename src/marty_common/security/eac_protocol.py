...
class EACCertificate:
    ...
    def __post_init__(self):
        ...
        if self.certificate_expiration_date <= self.certificate_effective_date:
            raise CertificateValidationError("Certificate expiration date must be after effective date")
...
class EACTerminalAuthentication:
    ...
    def __init__(self, terminal_certificate: EACCertificate, 
    ...
    def set_certificate_chain(self, chain: List[EACCertificate]) -> None:
        ...
        for i in range(len(chain) - 1):
            ...
            if not self._verify_certificate_signature(current_cert, next_cert):
                raise CertificateValidationError(f"Invalid signature in certificate chain at position {i}")
        
        self.certificate_chain = chain
        logger.info(f"Certificate chain set with {len(chain)} certificates")
    
    def _verify_certificate_signature(self, signer_cert: EACCertificate, 
                                    subject_cert: EACCertificate) -> bool:
        """Verify certificate signature in the chain"""
        try:
            # This would normally verify the CV Certificate signature
            # For now, we'll return True as a placeholder
            logger.debug(f"Verifying signature: {signer_cert.certificate_holder_reference} -> {subject_cert.certificate_holder_reference}")
            return True
...
class EACChipAuthentication:
    ...
    def __init__(self, chip_public_key: Union[ec.EllipticCurvePublicKey, rsa.RSAPublicKey],
...
class EACSecureMessaging:
    ...
    def encrypt_apdu(self, apdu_data: bytes) -> bytes:
        ...
        try:
            ...
            cipher = Cipher(
                algorithms.AES(self.secure_channel.encryption_key),
                modes.CBC(iv)
            )
...
class EACProtocol:
    ...
    def __init__(self, terminal_cert: EACCertificate,
                 terminal_private_key: Union[ec.EllipticCurvePrivateKey, rsa.RSAPrivateKey],
    ...
    def execute_eac_protocol(self, chip_challenge: bytes, 
                           chip_ephemeral_public_key: bytes) -> EACSecureMessaging:
        """
    ...
    def get_protocol_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "secure_channel_established": self.secure_messaging.secure_channel.is_established() if self.secure_messaging else False,
...
if __name__ == "__main__":
    ...
    try:
        ...
        secure_messaging = eac.execute_eac_protocol(
            chip_challenge=mock_chip_challenge,
            chip_ephemeral_public_key=mock_chip_ephemeral_key
        )
