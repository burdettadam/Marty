...
This module implements VDS-NC barcode generation and verification according to
...
class VDSNCGenerator:
    ...
    def __init__(self, signing_key: EllipticCurvePrivateKey, certificate_reference: str):
    ...
    def _create_header(self, issuing_country: str) -> str:
        ...
        if len(issuing_country) != 3:
            raise ValueError(f"Invalid country code: {issuing_country}")
...
class VDSNCVerifier:
    ...
    def __init__(self, public_keys: dict[str, EllipticCurvePublicKey]):
    ...
    def _create_cmc_from_payload(
        ...
        """
        from marty_common.models.passport import CMCData, CMCTD1MRZData, CMCSecurityModel
...
def generate_test_key_pair() -> tuple[EllipticCurvePrivateKey, EllipticCurvePublicKey]:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to generate key pair: {e!s}") from e
...
def export_public_key_pem(public_key: EllipticCurvePublicKey) -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to export public key: {e!s}") from e
...
def load_public_key_pem(pem_data: str) -> EllipticCurvePublicKey:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load public key: {e!s}") from e