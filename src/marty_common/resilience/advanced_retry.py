...
class AdvancedRetryManager:
        ...
        while attempt < self.config.max_attempts:
            ...
            try:
                ...
                logger.debug(
                    ...
                    attempt, self.config.max_attempts, self.name, exc
                )
        
        # All retries exhausted
        self.metrics.record_outcome(False, time.time() - start_time)
        raise RuntimeError(f"All {self.config.max_attempts} retry attempts exhausted for {self.name}")
    ...
    async def retry_async(self, func: Callable[..., Awaitable[T]], *args: Any, **kwargs: Any) -> T:
        ...
        # All retries exhausted
        self.metrics.record_outcome(False, time.time() - start_time)
        raise RuntimeError(f"All {self.config.max_attempts} async retry attempts exhausted for {self.name}")
