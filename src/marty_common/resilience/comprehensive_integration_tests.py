...
class ResilienceTestOrchestrator:
    ...
    def create_retry_manager(
        self, 
        name: str, 
        config: Optional[AdvancedRetryConfig] = None
    ) -> AdvancedRetryManager:
    ...
    @contextmanager
    def failure_injection(self, scenario: FailureScenario) -> Generator[None, None, None]:
    ...
    def generate_test_report(self) -> str:
        ...
        for i, result in enumerate(self.test_results, 1):
            ...
            if "circuit_breaker_rejections" in result:
                report_lines.append(f"Circuit breaker rejections: {result['circuit_breaker_rejections']}")
            ...
            success_rate = (
                ...
                if result["requests_attempted"] > 0 else 0.0
            )
...
FAILURE_SCENARIOS = [
    ...
    FailureScenario(
        name="total_failure",
        failure_rate=1.0,
        description="Complete service failure scenario"
    ),
]
