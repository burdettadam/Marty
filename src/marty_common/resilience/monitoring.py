...
class ResilienceMonitor:
    ...
    def get_circuit_breaker_health(self, name: str) -> ResilienceHealthCheck:
        ...
        if name not in self._circuit_breakers:
            return ResilienceHealthCheck(
                component_name=name,
                healthy=False,
                status="Circuit breaker not found"
            )
        ...
        failure_rate = (
            stats["total_failures"] / total_requests 
            if total_requests > 0 else 0.0
        )
        
        # Determine health based on state and failure rate
        healthy = (
            health_info["healthy"] and 
            failure_rate < self.circuit_breaker_failure_threshold
        )
    ...
    def get_retry_manager_health(self, name: str) -> ResilienceHealthCheck:
        ...
        if name not in self._retry_managers:
            return ResilienceHealthCheck(
                component_name=name,
                healthy=False,
                status="Retry manager not found"
            )
    ...
    def get_overall_health(self) -> dict[str, Any]:
        ...
        all_components_healthy = (
            all(h["healthy"] for h in circuit_breaker_health.values()) and
            all(h["healthy"] for h in retry_manager_health.values())
        )
