...
class GracefulDegradationManager:
    ...
    def set_degradation_level(self, level: DegradationLevel) -> None:
        ...
        logger.warning(
            "Service degradation level changed from %s to %s",
    ...
    async def execute_with_fallback(
        ...
        if not self.config.is_feature_enabled(feature):
            logger.info("Feature '%s' disabled at degradation level %s", feature, self.current_level.value)
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            if fallback_value is not None and feature not in manager.fallback_providers:
                manager.add_fallback_provider(
                    feature,
                    DefaultValueProvider(fallback_value)
                )
