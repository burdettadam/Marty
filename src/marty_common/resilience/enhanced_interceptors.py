...
class EnhancedResilienceInterceptor(grpc_aio.ServerInterceptor):
    ...
    def get_circuit_breaker_stats(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            method: breaker.stats()
            for method, breaker in self._circuit_breakers.items()
        }
        ...
        async def _wrapper(request, context):
            ...
            if not breaker.allow_request():
                ...
                await context.abort(
                    ...
                    f"Circuit breaker is open for {method}"
                )
            ...
            if self._should_inject_failure(context, method):
                ...
                self._log_request_details(method, False, duration, 
                                        type(injected_error).__name__, True)
