...
class ResilienceClientInterceptor(grpc.UnaryUnaryClientInterceptor):
    ...
    def __init__(
        ...
        self.retry_manager = AdvancedRetryManager(
            name=f"{service_name}_client",
            config=retry_config or AdvancedRetryConfig()
        )
    ...
    def _record_request_metrics(self, method: str, start_time: float, success: bool, error: Exception | None = None) -> None:
        ...
        if error:
            ...
            if isinstance(error, grpc.RpcError):
                try:
                    ...
                    self.metrics.increment_counter("grpc_client_errors_total", {
                        **labels, 
...
class AsyncResilienceClientInterceptor(grpc_aio.UnaryUnaryClientInterceptor):
    ...
    def __init__(
        ...
        # Initialize retry manager
        self.retry_manager = AdvancedRetryManager(
            name=f"{service_name}_async_client", 
            config=retry_config or AdvancedRetryConfig()
        )
    ...
    def _get_method_circuit_breaker(self, method: str) -> CircuitBreaker:
        ...
        if method not in self._method_circuit_breakers:
            ...
            self._method_circuit_breakers[method] = CircuitBreaker(f"{self.service_name}_{method}_async", config)
        return self._method_circuit_breakers[method]
    ...
    async def _record_request_metrics(self, method: str, start_time: float, success: bool, error: Exception | None = None) -> None:
        ...
        if error:
            ...
            if isinstance(error, grpc.aio.AioRpcError):
                try:
                    error_code = error.code().name
                    self.metrics.increment_counter("grpc_async_client_errors_total", {
                        **labels, 
                        "error_type": error_type,
                        "grpc_code": error_code
                    })
...
class EnhancedResilienceServerInterceptor(grpc.ServerInterceptor):
    ...
    def _record_server_metrics(self, method: str, start_time: float, success: bool, error: Exception | None = None) -> None:
        ...
        self.metrics.set_gauge("grpc_server_active_requests", self._active_requests, {"service": self.service_name})
