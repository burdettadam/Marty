...
from src.marty_common.service_registry import ServiceRegistry, ServiceDefinition
...
class EnvironmentConfigGenerator:
    ...
    def _get_service_dependencies(cls, service_name: str) -> List[str]:
        ...
        if service_name == "ui-app":
            dependencies.extend([
                "trust-svc", "csca-service", "document-signer",
                "passport-engine", "mdl-engine", "mdoc-engine"
            ])
    ...
    def _get_service_healthcheck(cls, service_def: ServiceDefinition) -> Dict[str, Any]:
        ...
        return {
            "test": [
                "CMD-SHELL",
                f"curl -f {service_def.health_endpoint} || exit 1"
            ],
            "interval": "30s",
            "timeout": "10s",
            "retries": 3,
    ...
    @classmethod
    def write_env_file(cls, environment: str, output_path: Optional[Path] = None) -> Path:
...
def main():
    ...
    parser.add_argument("environment", choices=["development", "production", "testing", "k8s:dev", "k8s:prod"])
    ...
    if args.format == "env":
        output_file = generator.write_env_file(args.environment, args.output_dir / f".env.{args.environment}")
