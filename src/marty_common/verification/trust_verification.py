...
class TrustResult:
    ...
    error_code: Optional[TrustValidationError] = None
...
class PKDResolver:
    ...
    def __init__(self, pkd_service_client=None, csca_service_client=None):
        """
    ...
    async def resolve_certificate(self, certificate_id: str) -> Optional[CertificateInfo]:
        """
    ...
    async def _resolve_from_pkd(self, certificate_id: str) -> Optional[CertificateInfo]:
        ...
        try:
            ...
            self.logger.error(f"Error resolving from PKD: {e}")
            return None
    
    async def _resolve_from_csca(self, certificate_id: str) -> Optional[CertificateInfo]:
        ...
        try:
            ...
            self.logger.error(f"Error resolving from CSCA: {e}")
            return None
    
    async def get_trust_anchors(self, country_code: str = None) -> List[CertificateInfo]:
        """
        ...
        try:
            ...
            self.logger.error(f"Error fetching trust anchors: {e}")
            return []
...
class CertificateChainValidator:
    ...
    def __init__(self, pkd_resolver: PKDResolver):
    ...
    async def validate_chain(
        ...
        intermediate_certificates: List[str] = None,
        trust_anchors: List[CertificateInfo] = None
    ) -> List[TrustResult]:
        """
    ...
    def _parse_certificate(self, certificate_data: str) -> Optional[CertificateInfo]:
        """
...
class TrustValidator:
    ...
    async def validate_trust(
        ...
        if validation_level in [TrustValidationLevel.STANDARD, TrustValidationLevel.STRICT]:
            pkd_results = await self._validate_pkd_resolution(
                issuing_authority, doc_class
            )
        ...
        if chip_data.get("security_object") or chip_data.get("sod"):
            ...
            all_results.append(TrustResult(
    ...
    async def _validate_chip_trust(
        ...
        if dsc_certificate:
            ...
            chain_results = await self.chain_validator.validate_chain(
                leaf_certificate=dsc_certificate,
