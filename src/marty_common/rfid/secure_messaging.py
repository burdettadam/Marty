...
class SecureMessaging:
    ...
    def derive_bac_keys(self, passport_number: str, date_of_birth: str, 
                       date_of_expiry: str) -> BACKeys:
    ...
    def perform_basic_access_control(self, bac_keys: BACKeys, 
    ...
    def derive_session_keys(self, bac_keys: BACKeys, rnd_ic: bytes, 
                          rnd_ifd: bytes) -> SessionKeys:
    ...
    def encrypt_command(self, apdu: bytes) -> bytes:
        ...
        mac_data = self._compute_mac(
            self.session_keys.k_s_mac,
            apdu[:4] + encrypted_data
        )
