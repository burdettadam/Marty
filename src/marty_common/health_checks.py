...
def create_http_health_check(url: str, timeout: float = 5.0, expected_status: int = 200) -> callable:
...
def create_resource_health_check(
    ...
    def check() -> bool:
        return (
            check_memory_usage(max_memory_percent) and
            check_cpu_usage(max_cpu_percent) and
            check_file_system(disk_path, min_disk_mb)
        )
    return check