...
from .role_separation import CryptoRole, KeyPurpose, KeyIdentity, create_evidence_key_identity
from .kms_provider import KMSManager, KeyOperation
...
class EvidenceMetadata:
    ...
    version: str = "1.0"
    signer_id: str = ""
    chain_hash: Optional[str] = None  # Hash of previous evidence for chaining
    correlation_id: Optional[str] = None  # For correlating related evidence
...
class EvidenceChain:
    ...
    def get_evidence_by_id(self, evidence_id: str) -> Optional[SignedEvidence]:
        """Find evidence by ID."""
...
class EvidenceVerifier:
    ...
    async def verify_evidence(self, signed_evidence: SignedEvidence) -> bool:
        ...
        try:
            ...
            # Get public key
            public_key_pem = await self.kms_manager.get_public_key_for_verification(
                key_identity=evidence_key_identity,
                requesting_role=CryptoRole.VERIFIER
            )
...
if __name__ == "__main__":
    import asyncio
    from .kms_provider import create_kms_manager, KMSProvider
