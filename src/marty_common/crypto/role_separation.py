...
class RoleKeyPolicy:
    ...
    max_key_lifetime_days: Optional[int]
    requires_audit: bool
...
class KeyIdentity:
    ...
    purpose: KeyPurpose
    key_id: str
    issuer_identifier: Optional[str] = None  # For issuing authorities
    device_identifier: Optional[str] = None  # For devices/wallets
    
    def __post_init__(self):
        """Validate key identity constraints."""
        policy = ROLE_POLICIES[self.role]
        if not policy.is_compatible_purpose(self.purpose):
            raise KeyPurposeMismatch(
                f"Purpose {self.purpose} not allowed for role {self.role}"
            )
...
class RoleSeparationEnforcer:
    ...
    def validate_key_operation(
        ...
        if operation == "sign" and key_identity.role in [CryptoRole.READER, CryptoRole.VERIFIER]:
            raise RoleBoundaryViolation(
                f"Reader/Verifier roles cannot perform signing operations"
            )
    ...
    def validate_key_sharing(
        source_role: CryptoRole,
        target_role: CryptoRole,
        ...
        if source_role in [CryptoRole.CSCA, CryptoRole.DSC]:
            if key_purpose in [KeyPurpose.CERTIFICATE_SIGNING, KeyPurpose.DOCUMENT_SIGNING]:
                raise RoleBoundaryViolation(
                    f"Private keys from {source_role} cannot be shared"
                )
