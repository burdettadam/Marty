...
from marty_common.crypto.vds_nc_keys import (
    ...
    KeyStatus, 
    VDSNCKeyMetadata,
    VDSNCKeyManager as BaseVDSNCKeyManager,
)
from marty_common.infrastructure.trust_models import VDSNCKeyModel, KeyRotationLog
...
class DatabaseVDSNCKeyManager:
    ...
    async def get_active_keys_for_verification(
        ...
        query = select(VDSNCKeyModel).where(
            ...
            VDSNCKeyModel.is_trusted == True,
            VDSNCKeyModel.distribution_enabled == True,
    ...
    async def get_private_key(self, kid: str) -> EllipticCurvePrivateKey | None:
        ...
        if not self.encryption_service:
            raise ValueError("Encryption service required for private key access")
        ...
        try:
            ...
            self.logger.exception(f"Error loading private key {kid}: {e}")
            return None
    ...
    async def _update_supersession(
        ...
        if not old_key or not new_key:
            raise ValueError("Both keys must exist for supersession")
