...
class CMCLDSGenerator:
    ...
    def _is_valid_image(self, image_bytes: bytes) -> bool:
        ...
        if image_bytes[:4] == b"\x00\x00\x00\x0C":
            return True
...
class CMCSODGenerator:
    ...
    def __init__(self, document_signer_service=None):
    ...
    def update_cmc_with_lds_data(
        ...
        try:
            ...
            if not cmc_certificate.uses_chip_security:
                raise ValueError("LDS data only applicable for chip-based security model")
