...
from .models import (
    TrustAnchor, DSCCertificate, CRLCache, RevokedCertificate,
    TrustSnapshot, JobExecution, RevocationStatus
)
...
class DatabaseManager:
    ...
    async def add_trust_anchor(self, trust_anchor_data: dict[str, Any]) -> str:
        ...
        async with self.get_session() as session:
            ...
            logger.info(f"Added trust anchor {trust_anchor_id} for {trust_anchor_data['country_code']}")
    ...
    async def update_dsc_revocation_status(
        ...
        async with self.get_session() as session:
            ...
            if updated:
                logger.info(f"Updated revocation status for {certificate_hash}: {revocation_status.value}")
    ...
    async def get_active_crls(self, issuer_dn: Optional[str] = None) -> list[dict[str, Any]]:
        """Get active CRLs."""
        async with self.get_session() as session:
            query = text("""
    ...
    async def start_job(self, job_name: str, job_type: str, metadata: Optional[dict[str, Any]] = None) -> str:
    ...
    async def validate_certificate_chain(self, certificate_hash: str) -> dict[str, Any]:
        ...
        async with self.get_session() as session:
            ...
            } if row else {"valid": False, "validation_errors": ["Certificate not found"]}
