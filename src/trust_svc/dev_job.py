...
class DevJobRunner:
    ...
    async def load_synthetic_master_list(
        ...
        try:
            ...
            await self.db_manager.complete_job(
                job_id=job_id,
                status="completed",
                records_processed=certificate_count
            )
            ...
            statistics = {
                ...
                "dsc_certificates": {
                    "loaded": added_dscs,
                    "revocation_status": revocation_stats
                },
                "crls": {
            ...
            self.logger.info(f"Synthetic data loaded for {country_code}: {certificate_count} certificates")
    ...
    def _generate_synthetic_crl(
        self,
        country_code: str,
        dsc_data_list: list[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _add_synthetic_crl(self, crl_data: Dict[str, Any]) -> None:
        ...
        async with self.db_manager.get_session() as session:
            ...
            result = await session.execute(revoked_query, {"country_code": crl_data["issuer_dn"][-3:]})
