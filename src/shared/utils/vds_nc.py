...
from src.shared.models.visa import Visa, VDSNCData, VisaType
...
class VDSNCEncoder:
    ...
    def encode_cbor(cls, header: Dict[str, Any], message: Dict[str, Any]) -> bytes:
        ...
        """
        if cbor2 is None:
            raise ImportError("cbor2 library required for CBOR encoding")
...
class VDSNCDecoder:
    ...
    def decode_cbor(cls, cbor_data: bytes) -> Dict[str, Any]:
        ...
        """
        if cbor2 is None:
            raise ImportError("cbor2 library required for CBOR decoding")
    ...
    def verify_signature(
        ...
        try:
            ...
            raise ImportError("cryptography library required for verification") from e
    ...
    @classmethod
    def decode_vds_nc(cls, barcode_data: str, public_key_pem: Optional[str] = None) -> Tuple[Dict[str, Any], bool]:
        """
        ...
        try:
            ...
            raise ValueError(f"Failed to decode VDS-NC data: {e}") from e
