...
from ..core import DeviceEngagement, mDLRequest, mDLResponse, create_device_engagement_qr
from ..crypto import KeyManager, SessionEncryption, DigitalSignature
from ..protocols import ISO18013_5Protocol, ProtocolState
from ..transport import create_transport, discover_devices, TransportInterface
...
class ReaderConfig:
    ...
    nfc_reader_name: Optional[str] = None
    https_base_url: Optional[str] = None
...
class ISO18013ReaderApp:
    ...
    def _load_or_generate_keys(self) -> None:
        ...
        try:
            ...
            if private_key_file.exists() and public_key_file.exists():
                ...
                # Generate new keys
                self.reader_private_key, self.reader_public_key = KeyManager.generate_ephemeral_keypair()
                
                # Save keys
                with open(private_key_file, 'wb') as f:
                    f.write(self.reader_private_key.private_bytes(
                ...
                with open(public_key_file, 'wb') as f:
                    f.write(self.reader_public_key.public_bytes(
                        encoding=serialization.Encoding.PEM,
    ...
    async def initiate_verification(self,
                                  transport_type: str,
                                  device_info: Optional[Dict[str, Any]] = None,
                                  requested_elements: Optional[List[str]] = None) -> str:
        """
        ...
        try:
            ...
            session_id = await self.protocol.initiate_reader_session(
                transport,
                {"org.iso.18013.5.1.mDL": requested_elements}
            )
    ...
    async def complete_verification(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.protocol.send_mdl_request(
                session_id,
                {"org.iso.18013.5.1.mDL": session_info["requested_elements"]}
            )
            ...
            self.logger.info(f"Verification completed: {session_id}, valid: {verification_result['valid']}")
    ...
    async def _perform_verification(self, 
        ...
        try:
            ...
            if self.config.verification_level in [VerificationLevel.STANDARD, VerificationLevel.ENHANCED]:
            ...
            if "birth_date" in verification_result["verified_data"]:
                try:
                    ...
                    birth_date = datetime.strptime(
                        verification_result["verified_data"]["birth_date"], 
                        "%Y-%m-%d"
                    ).date()
                    today = date.today()
                    ...
                    verification_result["warnings"].append("Could not calculate age from birth_date")
    ...
    def export_verification_report(self, 
        ...
        try:
            ...
            if format == "json":
                report_path = Path(self.config.key_storage_path) / f"verification_report_{timestamp}.json"
                ...
                import csv
                report_path = Path(self.config.key_storage_path) / f"verification_report_{timestamp}.csv"
                with open(report_path, 'w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        "Session ID", "Timestamp", "Transport", "Valid", 
                        "Verification Level", "Trust Status", "Errors"
                    ])
...
async def main():
    ...
    parser.add_argument("--transports", nargs="+", default=["ble", "nfc"], 
                       choices=["ble", "nfc", "https"], help="Supported transports")
    parser.add_argument("--verification-level", default="standard",
                       choices=["basic", "standard", "enhanced"], help="Verification level")
    parser.add_argument("--mode", default="interactive", 
                       choices=["interactive", "scan", "verify", "qr"], help="Operation mode")
                ...
                try:
                    ...
                    if command == "quit":
                        ...
                        for entry in history:
                            print(f"  {entry['session_id']}: {entry['result']['valid']} ({entry['transport_type']})")
                    elif command == "export":
