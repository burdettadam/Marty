...
from ..core import DeviceEngagement, mDLRequest, mDLResponse, SelectiveDisclosure
from ..crypto import KeyManager, SessionEncryption, DigitalSignature
from ..protocols import ISO18013_5Protocol, ProtocolState
from ..transport import create_transport, TransportInterface
from ..online import ISO18013_7Holder, ConsentRequest, ConsentResponse
...
class mDLCredential:
    ...
    birth_date: str
    expiry_date: str
    portrait: Optional[bytes] = None
    signature: Optional[bytes] = None
    
    # Additional elements
    issuing_date: Optional[str] = None
    driving_privileges: Optional[List[Dict[str, Any]]] = None
    ...
    def to_cbor(self) -> Dict[str, Any]:
        ...
        optional_fields = [
            "issuing_date", "administrative_number", "sex", "height", 
            "weight", "eye_color", "hair_color", "nationality", "resident_address"
        ]
...
class ConsentManager:
    ...
    async def request_consent(self, 
                            ...
                            requested_elements: Dict[str, List[str]],
                            session_id: str) -> ConsentResponse:
        """
        ...
        try:
            ...
            return ConsentResponse(
                approved=False,
                error=f"Consent failed: {e}",
                timestamp=datetime.utcnow()
            )
    
    async def _prompt_basic_consent(self, 
                                  reader_info: Dict[str, Any],
                                  requested_elements: Dict[str, List[str]]) -> ConsentResponse:
        ...
        try:
            ...
            return ConsentResponse(
                approved=False,
                error=f"Consent prompt failed: {e}",
                timestamp=datetime.utcnow()
            )
    
    async def _prompt_detailed_consent(self, 
                                     reader_info: Dict[str, Any],
                                     requested_elements: Dict[str, List[str]]) -> ConsentResponse:
        ...
        try:
                ...
                if response in ['y', 'yes']:
                    ...
                    try:
                        ...
                        if selected:
                            ...
                            namespace_approved = [elements[i] for i in indices if 0 <= i < len(elements)]
                    except:
            ...
            return ConsentResponse(
                approved=False,
                error=f"Detailed consent failed: {e}",
                timestamp=datetime.utcnow()
            )
    ...
    def audit_presentation(self, 
                         reader_info: Dict[str, Any],
                         presented_elements: Dict[str, List[str]],
                         session_id: str) -> None:
...
class ISO18013HolderApp:
    ...
    def _load_or_generate_keys(self) -> None:
        ...
        try:
            ...
            if private_key_file.exists() and public_key_file.exists():
                ...
                # Generate new keys
                self.holder_private_key, self.holder_public_key = KeyManager.generate_ephemeral_keypair()
                
                # Save keys
                with open(private_key_file, 'wb') as f:
                    f.write(self.holder_private_key.private_bytes(
                ...
                with open(public_key_file, 'wb') as f:
                    f.write(self.holder_public_key.public_bytes(
                        encoding=serialization.Encoding.PEM,
    ...
    async def start_passive_mode(self, transport_type: str) -> str:
        ...
        try:
            if transport_type == "ble":
                ...
                emulator = NFCCardEmulator(
                    card_name=self.config.nfc_card_name,
                    holder_app=self
                )
    ...
    async def handle_presentation_request(self, 
                                        session_id: str,
                                        reader_info: Dict[str, Any],
                                        requested_elements: Dict[str, List[str]]) -> bool:
        """
    ...
    def get_presentation_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return sorted(
            self.consent_manager.presentation_audit,
...
async def main():
    ...
    parser.add_argument("--consent-level", default="detailed",
    ...
    try:
        ...
        if args.mode == "passive_ble":
            ...
            for cred in credentials:
                print(f"  - {cred['document_type']}: {cred['holder_name']} ({cred['document_number']})")
                ...
                try:
                    ...
                    if command == "quit":
                        ...
                        for entry in history:
                            print(f"  {entry['timestamp']}: {entry['reader_org']} ({entry['element_count']} elements)")
                    else:
