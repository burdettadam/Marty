...
from ..apps.holder import ISO18013HolderApp, HolderConfig, ConsentLevel
...
class TestISO18013Protocol(unittest.TestCase):
    ...
    def test_mdl_request_creation(self):
        ...
        request = mDLRequest(
            ...
            reader_auth={
                "reader_key": self.generator.reader_public_key,
                "reader_cert_chain": []
            }
        )
    ...
    def test_mdl_response_creation(self):
        ...
        response = mDLResponse(
            documents={"org.iso.18013.5.1.mDL": disclosed_data},
            status="OK"
        )
...
class TestTestVectors(unittest.TestCase):
    ...
    def test_vector_generation(self):
        ...
        with open(Path(self.test_dir) / "test_summary.json", 'r') as f:
            summary = json.load(f)
    ...
    def test_vector_content_validation(self):
        ...
        with open(Path(self.test_dir) / "device_engagement.json", 'r') as f:
        ...
        required_fields = [
            "name", "description", "engagement_cbor", 
            "qr_content", "device_key_public", "expected_transports"
        ]
...
class TestSimulation(unittest.TestCase):
        ...
        try:
            result = loop.run_until_complete(
                self.simulator.simulate_mdl_transaction(
                    transport_type="mock",
                    requested_elements=["family_name", "birth_date"]
                )
            )
            ...
            expected_steps = [
                "transport_connect", "device_engagement", "session_establishment",
                "mdl_request", "mdl_response", "verification"
            ]
    ...
    def test_scenario_execution(self):
        ...
        try:
            result = loop.run_until_complete(
                self.simulator.run_test_scenario("basic_verification")
            )
...
async def test_protocol_compliance(test_vectors):
    ...
    # Load test vectors
    with open(Path(test_vectors.output_dir) / "complete_test_vectors.json", 'r') as f:
        vectors = json.load(f)
