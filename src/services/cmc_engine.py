...
from marty_common.utils.mrz_utils import generate_td1_mrz, parse_td1_mrz
from marty_common.event_bus.event_bus import EventBus
from marty_common.services.document_signer import DocumentSigner
from marty_common.policies.annex9_policies import get_policy_manager, BackgroundCheckStatus, VisaFreeEntryStatus
from proto import cmc_engine_pb2_grpc, cmc_engine_pb2
...
class CMCEngineServicer(cmc_engine_pb2_grpc.CMCEngineServicer):
    ...
    async def CreateCMC(
        ...
        try:
            ...
            logger.error(f"Error creating CMC: {str(e)}")
            return cmc_engine_pb2.CreateCMCResponse(
                success=False,
    ...
    async def GetCMC(
        ...
        try:
            ...
            logger.error(f"Error retrieving CMC: {str(e)}")
            return cmc_engine_pb2.CMCResponse(
                success=False,
                error_message=f"Internal error: {str(e)}"
            )
    ...
    async def SignCMC(
        ...
        try:
            ...
            logger.error(f"Error signing CMC: {str(e)}")
            return cmc_engine_pb2.SignCMCResponse(
                success=False,
                error_message=f"Internal error: {str(e)}"
            )
    ...
    async def GenerateVDSNCBarcode(
        ...
        try:
            ...
            logger.error(f"Error generating VDS-NC barcode: {str(e)}")
            return cmc_engine_pb2.GenerateVDSNCResponse(
                success=False,
                error_message=f"Internal error: {str(e)}"
            )
    ...
    async def VerifyCMC(
        ...
        try:
            ...
            logger.error(f"Error verifying CMC: {str(e)}")
            return cmc_engine_pb2.VerifyCMCResponse(
                success=False,
                error_message=f"Internal error: {str(e)}"
            )
    ...
    def _convert_verification_result_to_protobuf(
        self, result: dict[str, Union[str, bool]]
