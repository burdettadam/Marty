...
class ZoneFieldDataModel(BaseModel):
    ...
    def validate_zone(cls, v):
        ...
        valid_zones = {
            'VISUAL_OCR', 'MRZ', 'BARCODE_1D', 'BARCODE_2D', 
            'RFID_CHIP', 'MAGNETIC_STRIPE'
        }
...
class ConsistencyCheckRequestModel(BaseModel):
    ...
    zone_data: List[ZoneFieldDataModel] = Field(..., description="Data extracted from document zones")
    rules_to_check: List[str] = Field(default_factory=list, description="Specific rules to check (empty = all applicable)")
    ...
    fuzzy_match_threshold: float = Field(default=0.8, ge=0.0, le=1.0, description="Threshold for fuzzy matching")

    @validator('rules_to_check')
    def validate_rules(cls, v):
        """Validate rule names."""
        valid_rules = {
            'FIELD_EXACT_MATCH', 'FIELD_FUZZY_MATCH', 'DATE_FORMAT_VALIDATION',
            'CHECKSUM_VALIDATION', 'CROSS_REFERENCE_VALIDATION'
        }
        for rule in v:
...
class RuleCheckResultModel(BaseModel):
    ...
    mismatches: List[FieldMismatchModel] = Field(default_factory=list, description="Field mismatches found")
...
class ConsistencyCheckResponseModel(BaseModel):
    ...
    rule_results: List[RuleCheckResultModel] = Field(default_factory=list, description="Individual rule results")
    critical_mismatches: List[FieldMismatchModel] = Field(default_factory=list, description="Critical mismatches")
    warnings: List[FieldMismatchModel] = Field(default_factory=list, description="Warning-level mismatches")
...
class AuditHistoryRequestModel(BaseModel):
    ...
    audit_id: Optional[str] = Field(None, description="Specific audit record ID")
    request_id: Optional[str] = Field(None, description="Request ID filter")
    from_time: Optional[str] = Field(None, description="Start time filter (ISO format)")
    to_time: Optional[str] = Field(None, description="End time filter (ISO format)")
...
class ConsistencyEngineRESTAPI:
    ...
    def __init__(self, grpc_service: Optional[ConsistencyEngine] = None) -> None:
    ...
    def _setup_middleware(self) -> None:
        ...
        async def log_requests(request: Request, call_next):
            ...
            # Record metrics
            self.metrics.histogram("http_request_duration_ms", processing_time, {
                "method": request.method,
    ...
    def _setup_routes(self) -> None:
        ...
        async def health_check():
            ...
            return {"status": "healthy", "service": "consistency-engine", "timestamp": datetime.now().isoformat()}
        ...
        async def get_audit_history(
            audit_id: str,
            request: AuditHistoryRequestModel = None
        ):
            ...
            try:
                ...
                for rule in grpc_response.rules:
                    rule_model = RuleDescriptionModel(
                        ...
                        applicable_zones=[consistency_engine_pb2.DocumentZone.Name(zone) for zone in rule.applicable_zones],
                        applicable_fields=[consistency_engine_pb2.CanonicalField.Name(field) for field in rule.applicable_fields],
                ...
                rest_response = GetSupportedRulesResponseModel(
                    rules=rules,
                    total_rules=grpc_response.total_rules
                )
    ...
    def _rest_to_grpc_consistency_request(self, rest_request: ConsistencyCheckRequestModel) -> consistency_engine_pb2.ConsistencyCheckRequest:
        """Convert REST request to gRPC request."""
    ...
    def _grpc_to_rest_consistency_response(self, grpc_response: consistency_engine_pb2.ConsistencyCheckResponse) -> ConsistencyCheckResponseModel:
        """Convert gRPC response to REST response."""
        ...
        for rule_result in grpc_response.rule_results:
            result_model = RuleCheckResultModel(
                ...
                confidence_level=consistency_engine_pb2.ConfidenceLevel.Name(rule_result.confidence_level),
        ...
        rest_response = ConsistencyCheckResponseModel(
            ...
            overall_status=consistency_engine_pb2.ConsistencyStatus.Name(grpc_response.overall_status),
            ...
            overall_confidence_level=consistency_engine_pb2.ConfidenceLevel.Name(grpc_response.overall_confidence_level),
...
def create_consistency_engine_app(grpc_service: Optional[ConsistencyEngine] = None) -> FastAPI:
