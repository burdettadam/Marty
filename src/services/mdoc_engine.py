...
class MDocEngineServicer(mdoc_engine_pb2_grpc.MDocEngineServicer):
    ...
    def EstablishSession(self, request, context):
        ...
        try:
            ...
            if request.transport_protocol not in valid_transports:
                ...
                context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                context.set_details(
                    f"Invalid transport protocol. Must be one of: {valid_transports}"
                )
                return mdoc_engine_pb2.EstablishSessionResponse(
                    session_id="",
                    success=False,
            ...
            return mdoc_engine_pb2.EstablishSessionResponse(
                session_id="", success=False, error_message=f"Session establishment failed: {e}"
            )
        ...
        try:
            if not request.session_id:
                ...
                return mdoc_engine_pb2.ProcessMDocRequestResponse(
                    success=False, error_message="session_id is required"
                )
            ...
            try:
                ...
                return mdoc_engine_pb2.ProcessMDocRequestResponse(
                    success=False, error_message=f"Invalid request_data format: {e}"
                )
            ...
            mdoc_response = {
                ...
                "issuer_signed": {
                    "name_spaces": {},
                    "issuer_auth": {"x5chain": ["mock_issuer_cert"], "algorithm": "ES256"},
                },
                "status": 0,
            }

            # Log success with visual indicators
            self.logger.info("✓ mDoc request processed successfully")
            ...
            return mdoc_engine_pb2.ProcessMDocRequestResponse(
                success=True, response_data=json.dumps(mdoc_response).encode("utf-8")
            )
            ...
            return mdoc_engine_pb2.ProcessMDocRequestResponse(
                success=False, error_message=f"Request processing failed: {e}"
            )

    def _generate_device_engagement_data(
        self, session_id: str, method: str, transport: str
    ) -> dict:
        ...
        if method == "QR_CODE":
            ...
            self.logger.info("✓ Generated QR code engagement data")

        elif method == "NFC":
            nfc_data = self._generate_nfc_data(session_id, transport)
            base_engagement["nfc"] = nfc_data
            self.logger.info("✓ Generated NFC engagement data")
            ...
            self.logger.info("✓ Generated Bluetooth engagement data")
            ...
            self.logger.info("✓ Generated WiFi Aware engagement data")
    ...
    def _get_disclosure_policy(self, doc_type: str) -> dict:
        ...
        policies = {
            ...
            "default": {"basic": ["family_name", "given_name"], "full": ["*"]},
        }
