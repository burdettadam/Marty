...
class TD2DocumentDataRequest(BaseModel):
    ...
    place_of_issue: str | None = Field(None, description="Place of issue")
...
class PolicyConstraintsRequest(BaseModel):
    ...
    employment_sectors: list[str] | None = Field(None, description="Allowed employment sectors")
    max_stay_duration: int | None = Field(None, description="Maximum stay in days")
    renewable: bool | None = Field(True, description="Whether document is renewable")
...
class CreateTD2DocumentRequest(BaseModel):
    ...
    policy_constraints: PolicyConstraintsRequest | None = None
...
class TD2MRZDataResponse(BaseModel):
    ...
    check_digit_dob: str | None = None
    check_digit_expiry: str | None = None
    check_digit_composite: str | None = None
...
class TD2DocumentResponse(BaseModel):
    ...
    mrz_data: TD2MRZDataResponse | None = None
    status: str
    security_model: str
    metadata: dict[str, str] | None = None
...
class TD2RestAPI:
    ...
    def _setup_routes(self) -> None:
        ...
        async def create_document(request: CreateTD2DocumentRequest):
            ...
            try:
                ...
                logger.exception(f"Unexpected error in create_document: {e}")
        ...
        async def issue_document(document_id: str, generate_chip_data: bool = False):
            ...
            try:
                ...
                logger.exception(f"Unexpected error in issue_document: {e}")
                raise HTTPException(status_code=500, detail=f"Internal error: {e!s}")
            else:
                return response
        ...
        async def verify_document(request: VerifyTD2DocumentRequest):
            ...
            try:
                ...
                logger.exception(f"Unexpected error in verify_document: {e}")
                raise HTTPException(status_code=500, detail=f"Internal error: {e!s}")
            else:
                return response
        ...
        async def get_document(document_id: str, include_chip_data: bool = False):
            ...
            try:
                ...
                logger.exception(f"Unexpected error in get_document: {e}")
                raise HTTPException(status_code=500, detail=f"Internal error: {e!s}")
            else:
                return response
        ...
        async def search_documents(request: SearchTD2DocumentsRequest):
            ...
            try:
                ...
                response = {
                    "documents": [
                        self._convert_document_response(doc) for doc in search_response.documents
                    ],
                ...
                logger.info(
                    f"TD-2 search completed via REST: {len(search_response.documents)} results"
                )
            except TD2ServiceError as e:
        ...
        async def update_status(document_id: str, request: UpdateStatusRequest):
            ...
            try:
                ...
                document = await self.td2_service.update_status(
                    document_id, status_enum, request.reason
                )
                response = self._convert_document_response(document)
                ...
                logger.exception(f"Invalid status: {e}")
                raise HTTPException(status_code=400, detail=f"Invalid status: {e!s}")
            except Exception as e:
                logger.exception(f"Unexpected error in update_status: {e}")
                raise HTTPException(status_code=500, detail=f"Internal error: {e!s}")
            else:
                return response
        ...
        async def revoke_document(document_id: str, reason: str):
            ...
            try:
                ...
                logger.exception(f"Unexpected error in revoke_document: {e}")
                raise HTTPException(status_code=500, detail=f"Internal error: {e!s}")
            else:
                return response
        ...
        async def renew_document(document_id: str, request: RenewDocumentRequest):
            ...
            try:
                ...
                if request.updated_constraints:
                    updated_constraints = self._convert_policy_constraints(
                ...
                logger.exception(f"Unexpected error in renew_document: {e}")
                raise HTTPException(status_code=500, detail=f"Internal error: {e!s}")
            else:
                return response
        ...
        async def get_expiring_documents(
            ...
            try:
                ...
                logger.exception(f"Unexpected error in get_expiring_documents: {e}")
                raise HTTPException(status_code=500, detail=f"Internal error: {e!s}")
            else:
                return response
    ...
    def _convert_create_request(
        self, request: CreateTD2DocumentRequest
    ) -> TD2DocumentCreateRequest:
    ...
    def _convert_verify_request(
        self, request: VerifyTD2DocumentRequest
    ) -> TD2DocumentVerifyRequest:
    ...
    def _convert_search_request(
        self, request: SearchTD2DocumentsRequest
    ) -> TD2DocumentSearchRequest:
    ...
    def _convert_document_response(
        self, document: TD2Document, include_chip_data: bool = False
    ) -> TD2DocumentResponse:
    ...
    def _convert_document_type_string(self, type_str: str) -> TD2DocumentType:
        ...
        if type_str not in type_map:
            msg = f"Invalid document type: {type_str}"
    ...
    def _convert_status_string(self, status_str: str) -> TD2Status:
        ...
        if status_str not in status_map:
            msg = f"Invalid status: {status_str}"
            raise ValueError(msg)
    ...
    def _convert_security_model_string(self, model_str: str) -> SecurityModel:
        ...
        if model_str not in model_map:
            msg = f"Invalid security model: {model_str}"
            raise ValueError(msg)
...
def run_td2_rest_api(td2_service: TD2Service, host: str = "0.0.0.0", port: int = 8080) -> None:
    """
