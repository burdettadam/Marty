...
class PkdDownloadManifest(Base):
    ...
    download_completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class PkdCertificateEntry(Base):
    ...
    certificate_type: Mapped[str] = mapped_column(String(64), nullable=False, index=True)  # CSCA, DS, etc.
    ...
    last_download_attempt: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class PkdSyncJob(Base):
    ...
    job_type: Mapped[str] = mapped_column(String(64), nullable=False, index=True)  # FULL_SYNC, INCREMENTAL, etc.
    countries: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)  # Specific countries or None for all
