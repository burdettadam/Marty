...
class CscaCertificate(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    certificate_id: Mapped[str] = mapped_column(String(128), unique=True, nullable=False, index=True)
    ...
    trust_status: Mapped[str] = mapped_column(String(32), nullable=False, default="TRUSTED", index=True)
    trust_updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class CertificateChain(Base):
    ...
    chain_certificates: Mapped[list[str]] = mapped_column(JSON, nullable=False)  # List of cert IDs in order
    validation_status: Mapped[str] = mapped_column(String(32), nullable=False, default="VALID", index=True)
    validation_errors: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    last_validated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    usage_purpose: Mapped[str] = mapped_column(String(64), nullable=False)  # DOCUMENT_SIGNER, PASSPORT, etc.
    trust_status: Mapped[str] = mapped_column(String(32), nullable=False, default="TRUSTED", index=True)
    extra_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(timezone.utc)
...
class CrlCache(Base):
    ...
    download_status: Mapped[str] = mapped_column(String(32), nullable=False, default="SUCCESS", index=True)
...
class OcspCache(Base):
    ...
    certificate_status: Mapped[str] = mapped_column(String(32), nullable=False, index=True)  # GOOD, REVOKED, UNKNOWN
    ...
    signature_valid: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    query_status: Mapped[str] = mapped_column(String(32), nullable=False, default="SUCCESS", index=True)
