...
class MDLCertificateMonitor:
    ...
    def _assess_expiry_impact(self, cert: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "business_impact": "Service disruption" if cert["status"] == "expired" else "Potential service disruption",
            "recommended_actions": [
            ...
            "estimated_downtime": "0-2 hours" if cert["status"] != "expired" else "Until certificate renewal"
        }
    ...
    def get_certificate_status_overview(self) -> dict[str, Any]:
        ...
        return {
            ...
            "critical_alerts": len([a for a in self.expiry_alerts.values() if a["alert_level"] == "critical"]),
            "expired_certificates": len([c for c in self.dsc_certificates.values() if c["status"] == "expired"]),
            "certificates_needing_renewal": len([
                c for c in self.dsc_certificates.values() 
    ...
    def get_expiry_dashboard(self) -> dict[str, Any]:
        ...
        sorted_certs = sorted(
            self.dsc_certificates.values(),
            key=lambda x: x["not_after"]
        )
        ...
        for cert_id, cert in self.dsc_certificates.items():
            ...
            if cert["status"] in ["expired", "critical", "expiring_soon"]:
                ...
                recommendation = {
                    "certificate_id": cert_id,
                    "priority": priority,
                    "action": "immediate_renewal" if cert["status"] == "expired" else "schedule_renewal",
