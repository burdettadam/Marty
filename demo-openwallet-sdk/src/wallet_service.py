...
class ImportCredentialRequest(BaseModel):
    credential_offer_uri: str | None = Field(
        default=None, description="OpenID4VCI credential offer URI"
    )
    credential_data: str | None = Field(default=None, description="Raw credential data")
    import_method: str = Field(
        default="openid4vci", description="Import method: openid4vci, qr_code, or direct"
    )
...
class PresentationRequest(BaseModel):
    ...
    selective_disclosure: dict[str, bool] | None = Field(
        default=None, description="Selective disclosure preferences"
    )
...
class MultipazWalletSDK:
    ...
    @staticmethod
    async def store_credential(credential_data: dict[str, Any], user_consent: bool = True) -> str:
    ...
    async def create_presentation(
        credential_ids: list[str], presentation_definition: dict[str, Any], nonce: str
    ) -> dict[str, Any]:
        ...
        presentation = {
            ...
            "presentation_submission": {
                ...
                "descriptor_map": [{"id": "credential_1", "format": "mso_mdoc", "path": "$"}],
            },
    ...
    @staticmethod
    async def establish_proximity_session(verifier_engagement: dict[str, Any]) -> dict[str, Any]:
    ...
    @staticmethod
    async def import_credential_from_offer(credential_offer_uri: str) -> dict[str, Any]:
...
async def get_wallet_status(user_id: str, conn=Depends(get_db_connection)):
    """Get wallet status for a user"""

    # Count user's credentials
    mdoc_count = await conn.fetchval(
        "SELECT COUNT(*) FROM credentials.mdocs WHERE user_id = $1 AND status = 'ISSUED'", user_id
    )

    mdl_count = await conn.fetchval(
        "SELECT COUNT(*) FROM credentials.mdls WHERE user_id = $1 AND status = 'ISSUED'", user_id
    )
...
async def list_wallet_credentials(user_id: str, conn=Depends(get_db_connection)):
...
async def get_credential_details(credential_id: str, conn=Depends(get_db_connection)):
...
async def import_credential(
    user_id: str, request: ImportCredentialRequest, conn=Depends(get_db_connection)
):
    ...
    try:
        ...
        credential_id = await MultipazWalletSDK.store_credential(
            credential_data=credential_data, user_consent=True
        )
        ...
        if credential_data.get("doctype") == "org.iso.18013.5.1.mDL":
            ...
            await conn.execute(
                ...
                user_id,
                "IMPORTED_DOCUMENT",
                credential_data["claims"].get(
                    "document_number", f"IMPORTED_{uuid.uuid4().hex[:8]}"
                ),
                "ISSUED",
                json.dumps(credential_data["claims"]),
                json.dumps(credential_data),
...
async def create_openid4vp_presentation(request: PresentationRequest):
    ...
    try:
        ...
        presentation_definition = {
            "id": "demo_request",
            "input_descriptors": [
                {"id": "credential_1", "format": {"mso_mdoc": {"alg": ["ES256"]}}}
            ],
        }
...
async def create_proximity_presentation(request: ProximityPresentationRequest):
    ...
    try:
            ...
            "input_descriptors": [
                {
                    ...
                    "constraints": {
                        "fields": [{"path": [f"$.{claim}"]} for claim in request.requested_claims]
                    },
                }
            ],
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create proximity presentation: {str(e)}"
        )


@app.delete("/api/wallet/credentials/{credential_id}")
async def delete_credential(credential_id: str, conn=Depends(get_db_connection)):
    ...
    try:
        ...
        mdoc_deleted = await conn.execute(
            "DELETE FROM credentials.mdocs WHERE mdoc_id = $1", credential_id
        )

        mdl_deleted = await conn.execute(
            "DELETE FROM credentials.mdls WHERE mdl_id = $1", credential_id
        )
...
async def provision_demo_credentials(user_id: str):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            mdl_response = await client.post(
                ...
                json={
                    ...
                    "driving_privileges": {"license_class": "C"},
                },
            )
        ...
        return {
            ...
            "credentials": [
                {
                    "type": "mDL",
                    "response": mdl_response.json() if mdl_response.status_code == 200 else None,
                },
                {
                    "type": "mDoc",
                    "response": mdoc_response.json() if mdoc_response.status_code == 200 else None,
                },
            ],
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to provision demo credentials: {str(e)}"
        )
