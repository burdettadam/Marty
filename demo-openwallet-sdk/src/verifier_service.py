...
SUPPORTED_CLAIMS = os.getenv('SUPPORTED_CLAIMS', 'given_name,family_name,birth_date,license_number,driving_privileges').split(',')
...
class CreatePresentationRequestModel(BaseModel):
    ...
    callback_url: Optional[str] = Field(default=None, description="Callback URL for async responses")
...
class MultipazVerifierSDK:
    ...
    @staticmethod
    async def establish_iso18013_session(
        device_engagement: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            "input_descriptors": [
                {
                    ...
                    "constraints": {
                        "fields": [
                            ...
                            {
                                "path": ["$.birth_date"],
                                "purpose": "Birth date verification"
                            }
                        ]
                    }
                }
...
async def readiness_check():
    ...
    try:
        ...
        return {"status": "ready", "database": "connected", "presentation_definitions": len(PRESENTATION_DEFINITIONS)}
    except Exception as e:
...
async def create_presentation_request(
    request: CreatePresentationRequestModel,
    ...
    try:
        ...
        expires_at = datetime.now(timezone.utc).replace(minute=datetime.now(timezone.utc).minute + 15)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create presentation request: {str(e)}")
...
async def get_presentation_request(
    ...
    session = await conn.fetchrow(
        "SELECT * FROM sessions.presentation_sessions WHERE session_id = $1",
        request_id
    )
...
async def verify_presentation(
    ...
    try:
        ...
        return VerificationResult(
            ...
            status=status,
            verified_claims=verification_result.get("claims") if verification_result["valid"] else None,
            errors=verification_result.get("errors", []) if not verification_result["valid"] else None,
