...
class DrivingPrivileges(BaseModel):
    ...
    restrictions: Optional[List[str]] = Field(default=None, description="License restrictions")
    endorsements: Optional[List[str]] = Field(default=None, description="License endorsements")
...
class CreateMDocRequest(BaseModel):
    ...
    expiry_date: Optional[str] = Field(default=None, description="Expiry date in YYYY-MM-DD format")
...
class CredentialResponse(BaseModel):
    ...
    document_type: str
    created_at: str
    multipaz_data: Optional[Dict[str, Any]] = None
    ...
    async def create_mdoc(
        ...
        mock_mdoc = {
            ...
            "issuerSigned": {
                ...
                "issuerAuth": {
                    "signature": f"mock_signature_{uuid.uuid4()}",
                    "algorithm": "ES256"
                }
            },
            "deviceSigned": {
                "deviceAuth": {
                    "deviceSignature": f"mock_device_sig_{uuid.uuid4()}"
                }
            },
            "multipaz_metadata": {
    ...
    async with db_pool.acquire() as conn:
        for user in sample_users:
            ...
            existing = await conn.fetchrow(
                "SELECT user_id FROM demo.sample_users WHERE user_id = $1",
                user["user_id"]
            )
...
async def create_mdoc(
    ...
    expiry_date = request.expiry_date or (datetime.now(timezone.utc) + timedelta(days=365*10)).date().isoformat()
    ...
    try:
        multipaz_data = await MultipazSDK.create_mdoc(
            doctype="org.iso.18013.5.1" if request.document_type == "DRIVER_LICENSE" else "org.example.identity",
            claims=claims,
...
async def create_mdl(
    ...
    credential_id = str(uuid.uuid4())
    issue_date = request.issue_date or datetime.now(timezone.utc).date().isoformat()
    expiry_date = request.expiry_date or (datetime.now(timezone.utc) + timedelta(days=365*5)).date().isoformat()
    
    # Check if license number already exists
    existing = await conn.fetchrow(
        "SELECT mdl_id FROM credentials.mdls WHERE license_number = $1",
...
async def get_credential(
    ...
    credential = await conn.fetchrow(
        "SELECT * FROM credentials.mdocs WHERE mdoc_id = $1",
        credential_id
    )
    ...
    if not credential:
        # Try mDL
        credential = await conn.fetchrow(
            "SELECT * FROM credentials.mdls WHERE mdl_id = $1",
            credential_id
        )
