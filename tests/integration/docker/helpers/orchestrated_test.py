...
This module provides a base class for integration tests that work with
the TestOrchestrator system instead of managing Docker services directly.
"""
...
class OrchestratedIntegrationTest(unittest.TestCase):
            ...
            try:
                with self.get_grpc_channel(service_name) as channel:
                    try:
                        ...
                        logger.debug(
                            "Service %s connection failed: %s, retrying in %ds",
                            service_name,
                            e,
                            delay,
                        )
    ...
    def wait_for_any_service_ready(self, service_names: list, timeout: int = 30):
        ...
        while time.time() - start_time < timeout:
            for service_name in service_names:
                if service_name in self.SERVICE_PORTS and self.wait_for_service_ready(
                    service_name, timeout=5
                ):
    ...
    def skip_if_no_services(self, required_services: list):
        ...
        """
        available_services = [
            service for service in required_services if service in self.SERVICE_PORTS
        ]

        if not available_services:
            self.skipTest(f"No required services available: {required_services}")
