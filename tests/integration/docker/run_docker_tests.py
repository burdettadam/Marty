def check_docker_available():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Docker Compose is available (docker compose format): {compose_result.stdout.strip()}")
            return True, "compose"
            ...
            try:
                ...
                logger.info(f"Docker Compose is available (legacy format): {compose_result.stdout.strip()}")
        ...
        logger.error("Docker command not found. Please ensure Docker is installed and in your PATH.")
        return False, None
#!/usr/bin/env python3
"""
...
using the improved test directory structure.
...
def run_docker_tests(args):
    ...
    # Import base test class here to avoid circular imports
    ...
    try:
        ...
        if args.service:
            ...
            e2e_test_suite = test_loader.discover(
                str(e2e_dir),
                pattern="test_*.py"
            )
        ...
        # Clean up Docker services unless --no-cleanup is specified
        if not args.no_cleanup:
            logger.info("Cleaning up Docker services...")
            BaseDockerIntegrationTest.shutdown_docker_services()
        else:
            logger.info("Skipping cleanup due to --no-cleanup flag")


def main():
    """Main entry point for the Docker test runner."""
    parser = argparse.ArgumentParser(description='Run Docker-based integration tests')
    parser.add_argument('--skip-proto-compile', action='store_true', help='Skip compiling proto files')
    parser.add_argument('--no-cleanup', action='store_true', help='Do not clean up Docker services after tests')
    ...
    if not docker_available:
        logger.error("Docker check failed. Please ensure Docker and Docker Compose are installed and running.")
        sys.exit(1)
