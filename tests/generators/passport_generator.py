...
from src.marty_common.models.passport import Passport, DataGroup, DataGroupType
...
class PassportGenerator:
    ...
    def _format_dg2_bytes(self, image_data: bytes) -> bytes:
        ...
        # Combine all parts
        content = biometric_header + biometric_info + biometric_data + biometric_data_header + image_data
        dg2_bytes = tag + self._encode_length(len(content)) + content
        
        return dg2_bytes
    
    def create_signature_data(self, 
                            dg1: Dict[str, Any], 
                            dg2: Dict[str, Any], 
                            cert_path: str, 
    ...
    def generate_passport(self, 
        ...
        sod = None
        if dg2 and cert_path and key_path and os.path.exists(cert_path) and os.path.exists(key_path):
