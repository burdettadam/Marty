...
def test_certificate_lifecycle():
    ...
    assert original_status_response.status in ["SUPERSEDED", "VALID"], \
    ...
    assert revoked_status_response.revocation_reason == "KEY_COMPROMISE", \
    ...
    logger.info(f"Found revoked certificate in the list with total count: {len(revoked_list_response.certificates)}")
