...
class MockCscaService:
    ...
    def GetCertificateStatus(self, request):
        ...
        not_after = datetime.fromisoformat(cert_data.get("not_after", datetime.now(timezone.utc).isoformat()))
    ...
    def ListCertificates(self, request):
        ...
        for cert_id, cert_data in self.certificates.items():
            ...
            if request.subject_filter and request.subject_filter.lower() not in cert_data.get("subject", "").lower():
                continue
                
            # Add to result list
            certificates.append(MockCertificateSummary(
        ...
        logger.info(f"Listing certificates with filter '{request.status_filter}', found: {len(certificates)}")
    ...
    def CheckExpiringCertificates(self, request):
        ...
        for cert_id, cert_data in self.certificates.items():
            ...
            if not_after <= expiry_date:
                expiring_certificates.append(MockCertificateSummary(
                    certificate_id=cert_id,
                    subject=cert_data.get("subject", ""),
                    status=cert_data.get("status", ""),
                    not_before=cert_data.get("not_before", ""),
        ...
        logger.info(f"Found {len(expiring_certificates)} certificates expiring within {request.days_threshold} days")
...
def test_certificate_lifecycle():
    ...
    assert create_response.status == "ISSUED", f"Expected status ISSUED, got {create_response.status}"
    ...
    logger.info(f"Found certificate in the list with total count: {len(list_response.certificates)}")
    ...
    assert renew_response.certificate_id != certificate_id, "Renewed certificate should have a new ID"
    assert renew_response.status == "RENEWED", f"Expected status RENEWED, got {renew_response.status}"
    ...
    assert original_status_response.status == "SUPERSEDED", f"Expected status SUPERSEDED, got {original_status_response.status}"
    ...
    assert revoke_response.status == "REVOKED", f"Expected status REVOKED, got {revoke_response.status}"
    ...
    assert revoked_status_response.status == "REVOKED", f"Expected status REVOKED, got {revoked_status_response.status}"
    assert revoked_status_response.revocation_reason == "KEY_COMPROMISE", f"Expected reason KEY_COMPROMISE, got {revoked_status_response.revocation_reason}"
    ...
    logger.info(f"Found expiring certificate in the list with total count: {len(expiry_response.certificates)}")
