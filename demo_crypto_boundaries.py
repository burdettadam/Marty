...
from marty_common.crypto.role_separation import (
    CryptoRole, KeyPurpose, RoleBoundaryViolation,
...
from marty_common.crypto.kms_provider import (
    KMSProvider, create_kms_manager
)
...
async def demonstrate_kms_provider():
    ...
    for i, log in enumerate(kms.audit_logs[-3:], 1):  # Show last 3 entries
        print(f"  {i}. {log.operation.value} on {log.key_identity.role.value} key - {'SUCCESS' if log.success else 'FAILED'}")
...
async def demonstrate_evidence_signing(kms, evidence_key_material):
    ...
    if len(chain.entries) >= 2:
        ...
        print(f"  🔗 Entry 2 chain hash: {entry2.evidence.metadata.chain_hash[:16] if entry2.evidence.metadata.chain_hash else 'None'}...")
